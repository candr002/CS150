...
int main()
{
    ...
    ///"Priming" loop was causing issues reading and calculating all lines of input for "codedog.txt". Attempted to
    /// re-download file with to success.
    /// Attempted changing getline statement for file input to cin which provided no change in loop behavior.
    /// Attempted multiple iterations of the while loop (priming with all three
    /// variables, partially priming with both one and two variables and an unprimed
    /// loop. The latter two options worked properly. I elected to leave the loop
    /// unprimed for the sake of limiting my input calls.
